...
export function commandUserRoutes(hearManager: HearManager<MessageContext>): void { 
  ...
  hearManager.hear(/!спутник|!Спутник|\/sputnik/, async (context: any) => {
    ...
    // lобавление подписи, если не подписан на канал
    let channelNote = ''
    try {
    ...
    await Send_Message(
      context,
      `🛰 Вы в системе поиска соролевиков, ${context.chat.firstName}, что изволите?${channelNote}`,
      keyboard
    )
  ...
  hearManager.hear(/!помощь|\/help/, async (context: any) => {
    ...
    \n🧭 Браузер — произвольный поиск по любым ключевым словам;
    ...
    \n@vasochka_s_konfetami — по вопросам разбана.
    \n@sputnik_signal — пользуясь ботом, вы подтверждаете, что ознакомились с информацией, размещенной в нашем официальном канале.
    \n@poiskcorola — лучший тг чат, в котором вы можете найти сорола.
    `)
...
hearManager.hear(/🧭 Браузер|!браузер|!Браузер|\/browser/, async (context: any) => {
    if (context.chat.id < 0) { return }
    const user_check = await prisma.account.findFirst({ where: { idvk: context.chat.id } })
    if (!user_check) { return }
    await Online_Set(context)
    ...
    await Send_Message(context, `🧭 Браузер — поиск анкет по ключевым словам`, keyboard)
